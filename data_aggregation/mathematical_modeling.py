...
class TestMathematicalModels(unittest.TestCase):
    ...
    def test_mathematical_models(self):
        ...
        ymax_KS = [False for f in demographiKS_out_path]
        ...
        make_Tc_Ks_MathModel_fig_with_subplots(bin_sizes_Ks, bin_sizes_Tc,
...
def make_Tc_Ks_MathModel_fig_with_subplots(bin_sizes_Ks, bin_sizes_Tc,
...
def plot_ks_model(this_ax, config_used, slim_ks_by_gene, spx_ks_by_gene, Ks_per_YR,
    ...
    if len(slim_ks_by_gene) > 0:
        n, bins, patches = this_ax.hist(slim_ks_by_gene, bins=bins, facecolor='b', alpha=0.25,
                     density=False)
    ...
    if len(spx_ks_by_gene) > 0:
        ...
        bin_midpoints=[0.5*(bins[i]+bins[i+1]) for i in range(0,len(bins)-1)]
        n, bins, patches = this_ax.hist(spx_ks_by_gene, bins=bins, facecolor='c', alpha=0.5,
                     density=False)
    ...
    ks_prediction_Tnow, ks_prediction_Tdiv, ks_model_with_dispersion = (
        predict_Ks(config_used.bottleneck_Ne, mean_ks_from_Tc, Ks_per_YR, bin_size, bins, config_used, num_slim_genes))
